...
const findAll = async () => {
  const sql = `SELECT * FROM form_nb_5`;
...
const findById = async (id) => {
  ...
  const sql = `SELECT * FROM form_nb_5 WHERE id = ?`;
...
const insert = async (formData) => {
  ...
  const nb5Id = result.insertId;

  return { ...formData, id: nb5Id };
};
...
const remove = async (id) => {
  if (Number.isNaN(id)) throw Error('Wrong Type');
  if (!id) throw Error('Missing Fields');
  const sql = `DELETE FROM form_nb_5 WHERE id = ?`;
  await query(sql, [id]);
